...
begin
 ...
 if p_op='send' then
  ...
  a:=p_data->'action';
  if jsonb_typeof(a)<>'object' or octet_length(a::text)>1024 or coalesce(a->>'type','') not in ('roll','choose','target','respond','next','vote') then raise exception '올바르지 않은 행동입니다.';end if;
  ...
  if jsonb_typeof(p_data->'state') is distinct from 'object' or octet_length((p_data->'state')::text)>100000 then raise exception '게임 상태를 확인해주세요.';end if;
