...
for(let seed=1;seed<=150;seed++){
 ...
 assert.equal(g.phase,'done');assert.equal(g.history.length,g.maxRounds+1);for(const p of g.players)assert.equal(p.choices.length,g.maxRounds);
}
...
g=transition(g,{actor:0,type:'vote',value:'care',expected:revision,round:4});g=transition(g,{actor:1,type:'vote',value:'care',expected:revision,round:4});assert.deepEqual(g.policies,['care']);assert.ok(['forecast','crisis','roll'].includes(g.phase));
